...
val LANGUAGE_REG = {
	STAR(
		("key" $ ("var" | "times" | "do" | "end" | "forward" | "backward" | "turn" | "by" | "degrees" | "left" | "right")) |
		("sym" $ RANGE(Set[Char]('=', '(', ')', '+', '-', '*', '/'))) | 
("id" $ ID)|
("int" $ INT)|
("db" $ DOUBLE)|
...
case class KWP(k: String) extends Parser[List[Token], Node] {
	def parse(tl: List[Token]) = {
		if (tl != Nil) tl match {
			case TurtlesTokenizer.T_KEY(s) :: ts if (k == s) => Set((KeyNode(k), ts))
			case TurtlesTokenizer.T_SYM(c) :: ts if (k == c.toString) => Set((KeyNode(k), ts))
			case _ => Set()
		}
		else Set()
	}
...
case class ElemParser(k: String) extends Parser[List[Token], String] {
	def parse(tl: List[Token]) = {
		if (tl != Nil) tl match {
			case TurtlesTokenizer.T_KEY(s) :: ts if (k == s) => Set((k, ts))
			case TurtlesTokenizer.T_SYM(c) :: ts if (k == c.toString) => Set((k, ts))
			case _ => Set()
		}
		else Set()
	}
...
case object IdParser extends Parser[List[Token], Node] {
  def parse(tl: List[Token]) = 
  	if (tl != Nil) tl match {
  		case TurtlesTokenizer.T_ID(s) :: ts => Set((IdNode(s), ts))
  		case _ => Set()
  	}
  	else Set()
}
...
case object IntParser extends Parser[List[Token], Node] {
	def parse(tl: List[Token]) = {
		if (tl != Nil) tl match {
			case TurtlesTokenizer.T_INT(n) :: ts => Set((IntNode(n), ts))
			case _ => Set()
		}
		else Set()
	}
...
case object DoubleParser extends Parser[List[Token], Node] {
	def parse(tl: List[Token]) = {
		if (tl != Nil) tl match {
			case TurtlesTokenizer.T_DB(db) :: ts => Set((DoubleNode(db), ts))
			case _ => Set()
		}
		else Set()
	}
...
case object StrParser extends Parser[List[Token], Node] {
	def parse(tl: List[Token]) = {
		if (tl != Nil) tl match {
			case TurtlesTokenizer.T_STR(s) :: ts => Set((StringNode(s), ts))
			case _ => Set()
		}
		else Set()
	}
...
case object CharParser extends Parser[List[Token], Node] {
	def parse(tl: List[Token]) = {
		if (tl != Nil) tl match {
			case TurtlesTokenizer.T_CHAR(c) :: ts => Set((CharNode(c), ts))
			case _ => Set()
		}
		else Set()
	}
...
case class TurtleProgramNode(ns: List[Node]) extends Node
case class StatementNode(ns: List[Node]) extends Node
case class VariableDeclarationNode(ns: List[Node]) extends Node
case class LoopNode(ns: List[Node]) extends Node
case class MoveNode(ns: List[Node]) extends Node
case class MoveCommandNode(k: String) extends Node
case class TurnNode(ns: List[Node]) extends Node
case class TurnCommandNode(k: String) extends Node
case class AdditionNode(ns: List[Node]) extends Node
case class MultiplicationNode(ns: List[Node]) extends Node
case class PrimaryNode(ns: List[Node]) extends Node



lazy val TurtleProgramParser: Parser[List[Token], Node] = {
	(CardiParser(StatementParser, C_STAR)).map{
		case n1 => TurtleProgramNode(List(n1))
	}
}

lazy val StatementParser: Parser[List[Token], Node] = {
	(MoveParser || TurnParser || LoopParser || VariableDeclarationParser).map{
		case n1 => StatementNode(List(n1))
	}
}

lazy val VariableDeclarationParser: Parser[List[Token], Node] = {
	(KWP("var") ~ IdParser ~ KWP("=") ~ IntParser).map{
		case _ ~ n1 ~ _ ~ _ => VariableDeclarationNode(List(n1))
	}
}

lazy val LoopParser: Parser[List[Token], Node] = {
	(AdditionParser ~ KWP("times") ~ KWP("do") ~ CardiParser(StatementParser, C_STAR) ~ KWP("end")).map{
		case n1 ~ _ ~ _ ~ n2 ~ _ => LoopNode(List(n1,n2))
	}
}

lazy val MoveParser: Parser[List[Token], Node] = {
	(MoveCommandParser ~ KWP("(") ~ AdditionParser ~ KWP(")")).map{
		case n1 ~ _ ~ n2 ~ _ => MoveNode(List(n1,n2))
	}
}

lazy val MoveCommandParser: Parser[List[Token], Node] = {
	(ElemParser("forward") || ElemParser("backward")).map[Node]{
		case s => MoveCommandNode(s)
	}
}
		
lazy val TurnParser: Parser[List[Token], Node] = {
	(KWP("turn") ~ TurnCommandParser ~ KWP("by") ~ DoubleParser ~ KWP("degrees")).map{
		case _ ~ n1 ~ _ ~ n2 ~ _ => TurnNode(List(n1,n2))
	}
}

lazy val TurnCommandParser: Parser[List[Token], Node] = {
	(ElemParser("left") || ElemParser("right")).map[Node]{
		case s => TurnCommandNode(s)
	}
}
		
lazy val AdditionParser: Parser[List[Token], Node] = {
	(MultiplicationParser ~ CardiParser(((KWP("+") || KWP("-")) ~ MultiplicationParser).map{case n1 ~ n2 => SeqNode(List(n1,n2))}, C_STAR)).map{
		case n1 ~ n2 => AdditionNode(List(n1,n2))
	}
}

lazy val MultiplicationParser: Parser[List[Token], Node] = {
	(PrimaryParser ~ CardiParser(((KWP("*") || KWP("/")) ~ PrimaryParser).map{case n1 ~ n2 => SeqNode(List(n1,n2))}, C_STAR)).map{
		case n1 ~ n2 => MultiplicationNode(List(n1,n2))
	}
}

lazy val PrimaryParser: Parser[List[Token], Node] = {
	(IntParser || IdParser || (KWP("(") ~ AdditionParser ~ KWP(")")).map{case _ ~ n1 ~ _ => n1}).map{
		case n1 => PrimaryNode(List(n1))
	}
}


def parse(s: String) : Node = {
	val tks = TurtlesTokenizer.tokenize(s)
	try {
		TurtleProgramParser.parse_all(tks).head
	} catch {
		case e: Exception => TurtleProgramNode(Nil)
	}
}
